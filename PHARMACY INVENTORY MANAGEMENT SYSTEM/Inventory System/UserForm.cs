...
namespace Inventory_System
{
    public partial class UserForm : Form
    {
        ...
        public void LoadUser()
        {
            int i = 0;
            dgvUser.Rows.Clear(); 
            cm = new SqlCommand("SELECT * FROM tblUser1", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void dgvUser_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Edit")
            {
                ...
                userModule.txtUserID.Text = dgvUser.Rows[e.RowIndex].Cells[4].Value.ToString();


                userModule.btnSave.Enabled = false;
                userModule.btnUpdate.Enabled = true;
                userModule.txtUserID.Enabled = false;
                userModule.ShowDialog();
            ...
            else if (colName == "Delete")
            {
                ...
                {
                    con.Open();
                    cm = new SqlCommand("DELETE FROM tblUser1 WHERE userID LIKE '" + dgvUser.Rows[e.RowIndex].Cells[4].Value.ToString() + " '", con);
                    cm.ExecuteNonQuery();
                    cm.Dispose();
                    con.Close();
