...
namespace Inventory_System
{
    public partial class LoginForm : Form
    {
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            try
            {
                cm = new SqlCommand("SELECT * FROM tblUser1 WHERE username=@username AND pass=@pass", con);
                ...
                if (dr.HasRows)
                {
                    MessageBox.Show("Welcome to Pharmacy Inventory Management System!", "ACCESS GRANTED", MessageBoxButtons.OK, MessageBoxIcon.Information);
